...
def keypoint_rcnn_loss(pred_keypoint_logits, instances, normalizer):
    ...
    if np.sum(np.isnan(pose3d_gt.detach().cpu().numpy())) == 0 :
        ...
        custom_plotting.plot_3Dpose(axs[1], pred_3d[-1].detach().cpu(),  bones=bones_ego, color_order=color_order_ego pred_integral['pose_2d global'],flip_yz=False)
        custom_plotting.plot_2Dpose(axs[2], pred_integral['pose_2d global'][-1],bones=bones_ego, color_order=color_order_ego,)
